...
void Cone::initializeConeBody()
{
	...
	vector<pntVertexData> v; // vertex positions		// TODO: pntVertexData
	...
	for( int i = 0; i < stacks-1; i++ )
    {
        for( int j=0; j < slices; j++)
        {
			...
			vec2 t0 = getPlanarTextCoords(v0,base, height);
			v.push_back( pntVertexData( v0, n01, t0 ) ); // 0				// TODO: pntVertexData for all pnVertexData
            
			vec3 v1 = vec3(cos(theta) * r1,   sin(theta)*r1,   z1  );
			vec2 t1 = getPlanarTextCoords(v1, base, height);
			v.push_back( pntVertexData( v1, n01, t1 ) ); // 1

			theta = (j+1)* angleStep;
			vec3 n23 = vec3(cos(theta) * cosn, sin(theta) * cosn, sinn);

            vec3 v2 = vec3(cos(theta) * r0,   sin(theta) * r0,   z0  );
			vec2 t2 = getPlanarTextCoords(v2, base, height);
			v.push_back( pntVertexData( v2, n23, t2 ) ); // 2
           
			vec3 v3 = vec3(cos(theta) * r1,   sin(theta) * r1,  z1  );
			vec2 t3 = getPlanarTextCoords(v3, base, height);
			v.push_back( pntVertexData( v3, n23, t3 ) ); // 3
	...
	for (int j=0; j < slices; j++)
	{
		...
		vec3 v2 = vec3( cos(theta) * r0, sin(theta)*r0, z0 );
		vec2 t2 = getPlanarTextCoords(v1, base, height);
...
void Cone::initializeConeBottom()
{
    ...
    for (int j = slices; j >= 0; j--) {			
		...
		vec2 t0 = vec2( v0.x, v0.y);
		v.push_back( pntVertexData( v0, n, t0 ));								// TODO: pntVertexData for all
		indices.push_back(currentIndex++);

		theta = (j-1)* angleStep;

		vec3 v1 = vec3( cos( theta ) * base, sin( theta ) * base, 0.0f);
		vec2 t1 = vec2( v1.x, v1.y );
		v.push_back( pntVertexData( v1, n, t1 ));
		indices.push_back(currentIndex++);

		v.push_back( pntVertexData( ctr, n, tr ));
		indices.push_back(currentIndex++);
	}
	...
	glVertexAttribPointer(3, 2, GL_FLOAT, GL_FALSE, sizeof(pntVertexData), (const GLvoid*)(sizeof(vec3) * 2) );
	glEnableVertexAttribArray(3);

...
void Cylinder::buildShaderProgram()
{
	...
	//material.setAmbientAndDiffuseMat( this->objectColor );
...
void Cylinder::initializeCylinderBody()
{
    ...
    for ( int i=1; i<=stacks; i++)
    {
        ...
        for (int j=0; j < slices; j++ ) {
			...
			vec2 t0 = getCylindericalTextCoords(theta, v0, height);
			v.push_back( pntVertexData( v0, n01, t0 ) ); // 0
			
			vec3 v1 = vec3(cos(theta)*radius, sin(theta)*radius, z1  );
			vec2 t1 = getCylindericalTextCoords(theta, v1, height);
			...
			vec2 t2 = getCylindericalTextCoords(theta, v2, height);
			v.push_back( pntVertexData( v2, n23, t2  ) ); // 2

            vec3 v3 = vec3(cos(theta)*radius, sin(theta)*radius, z1  );
			vec2 t3 = getCylindericalTextCoords(theta, v3, height);
...
void Cylinder::initializeCylinderBottom()
{
    ...
    for (int j = slices; j >=  0; j--) {
		...
		theta = j * angleStep;
		v.push_back( pntVertexData( vec3( cos(theta) * radius, sin(theta)*radius, 0.0f ), normal, getCylindericalTextCoords(theta, vec3(cos(theta)*radius, sin(theta)* radius, 0.0f), height)) ); 
		indices.push_back(currentIndex++);
...
void Cylinder::initializeCylinderTop()
{
    ...
    for (int j = 0; j <= slices; j++) {
		...
		theta = j * angleStep;
		v.push_back( pntVertexData( vec3( cos(theta) * radius, sin(theta)*radius, height ), normal, getCylindericalTextCoords(theta, vec3(cos(theta)*radius,sin(theta)*radius,height), height)) ); 
		indices.push_back(currentIndex++);

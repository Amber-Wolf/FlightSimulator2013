...
struct Material
{
	Material( GLint ambientMatLoc = 0xFFFFFFFF,  GLint diffuseMatLoc = 0xFFFFFFFF, 
			  ...
			  GLint toonLoc = 0xFFFFFFFF, GLint skLoc = 0xFFFFFFFF )
	{
	...
	{
		this->toon = on;
		if (toonLoc != 0xFFFFFFFF) {
			glUniform1i(toonLoc, toon);
		}
		else {
			//cout << "Toon enabled location not set." << endl;
		}
	}
	...
	{
		this->sky = on;
		if (skyLoc != 0xFFFFFFFF) {
			glUniform1i(skyLoc, sky);
		}
		else {
			//cout << "Sky enabled location not set." << endl;
		}
	}
